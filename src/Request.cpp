...
static std::string curlRequest(std::string method, std::string url, std::vector<std::string> headers, std::string params, std::string query)
{
  ...
  if (!curl)
  {
    Hook::Log("ERROR! Curl not initialized.");
    return "";
  }
  ...
  if (method.compare(Hook::Method::GET) == 0)
  {
    url = Hook::Str::format("%s?%s", url.c_str(), queryEsc);
  }
...
void Hook::Request::execute()
{
  Hook::Log("execute %s - url:%s params:%s query:%s", method.c_str(), url.c_str(), params.c_str(), query.c_str());
